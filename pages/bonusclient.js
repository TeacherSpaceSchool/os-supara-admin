        ...
        <App sorts={data.sortBonusClient} pageName={['admin', 'организация', 'менеджер'].includes(profile.role)?'Бонусы клиентов':'Мои бонусы'}>
            ...
            <div className={classes.page}>
                {list?list.map((element)=>
                    <LazyLoad scrollContainer={'.App-body'} key={element._id} height={height} offset={[height, 0]} debounce={100}  placeholder={<CardBonusClientPlaceholder height={height}/>}>
...
BonusClient.getInitialProps = async function() {
    return {
        data: await getBonusesClient({search: '', sort: '-createdAt'})
    };
};
