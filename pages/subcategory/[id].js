    ...
    return (
        <App filters={data.filterSubCategory} sorts={data.sortSubCategory} pageName={router.query.id==='all'?'Все':data.category!==null?data.category.name:'Ничего не найдено'}>
            <Head>
                ...
                <meta property="og:url" content={`https://${process.env.URL}/subcategory/${router.query.id}`} />
                <link rel='canonical' href={`https://${process.env.URL}/subcategory/${router.query.id}`}/>
            </Head>
