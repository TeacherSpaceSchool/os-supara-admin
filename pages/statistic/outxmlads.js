...
const OutXMLAds = React.memo((props) => {
    ...
    useEffect(()=>{
        (async()=>{
            setList(await outXMLAdsShoros({search: search}))
        })()
    },[sort, search])
    useEffect(()=>{
        (async()=>{
            setDistricts(await districtsOutXMLAdsShoros())
            setPagination(100)
            forceCheck()
        })()
    },[list])
    ...
    return (
        <App checkPagination={checkPagination} searchShow={true} pageName='Акционная интеграции 1С'>
            ...
            <div className={classes.page}>
                {districts.length>0?<CardOutXMLAds districts={districts} list={list} setList={setList}/>:null}
...
OutXMLAds.getInitialProps = async function(ctx) {
    ...
    return {
        data: {
            outXMLAdsShoros: await outXMLAdsShoros({search: ''}, ctx.req?await getClientGqlSsr(ctx.req):undefined),
            districts: await districtsOutXMLAdsShoros(ctx.req?await getClientGqlSsr(ctx.req):undefined)
        }
    };
};
