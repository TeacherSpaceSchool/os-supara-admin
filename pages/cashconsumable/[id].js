...
const CashConsumable = React.memo((props) => {
    ...
    let [amount, setAmount] = useState(data.cashConsumable?data.cashConsumable.amount:undefined);
    let [currencyType, setCurrencyType] = useState(data.cashConsumable?data.cashConsumable.currencyType:'сом');
    let handleCurrencyType = (async (event) => {
        setCurrencyType(event.target.value)
    })
    const { showLoad } = props.appActions;
    let [comment, setComment] = useState(data.cashConsumable?data.cashConsumable.comment:'');
    let [budget, setBudget] = useState(data.cashConsumable?data.cashConsumable.budget:'');
    let [supplier, setSupplier] = useState(data.cashConsumable?data.cashConsumable.supplier:undefined);
    return (
        <App pageName={router.query.id==='new'?'Добавить':data.cashConsumable?data.cashConsumable.number:'Ничего не найдено'}>
            <Head>
                <title>{router.query.id==='new'?'Добавить':data.cashConsumable?data.cashConsumable.number:'Ничего не найдено'}</title>
                <meta name='description' content='Система предназначена для ведения списка заявок на приобретение'/>
                <meta property='og:title' content={router.query.id==='new'?'Добавить':data.cashConsumable?data.cashConsumable.number:'Ничего не найдено'} />
            ...
            <Card>
                <CardContent className={classes.page}>
                    {
                        data.cashConsumable.number?
                            <div className={classes.row}>
                                <div className={classes.nameField}>Номер:</div>&nbsp;
                                <div className={classes.value}>{data.cashConsumable.number}</div>
                            </div>
                            :
                            null
                    }
                    {
                        data.cashConsumable.createdAt?
                            <div className={classes.row}>
                                <div className={classes.nameField}>Дата выплаты:&nbsp;</div>
                                <div className={classes.value}>{pdDDMMYYHHMM(data.cashConsumable.createdAt)}</div>
                            </div>
                            :
                            null
                    }
                    {
                        router.query.id==='new'?
                            ...
                            <div className={classes.row}>
                                ...
                                <div className={classes.value}>{data.cashConsumable.supplier.name}</div>
                            </div>
                            <div className={classes.row}>
            ...
            <div className={isMobileApp?classes.bottomDivM:classes.bottomDivD}>
                ...
                {
                    router.query.id!=='new'&&'обработка'!==data.cashConsumable.status?
